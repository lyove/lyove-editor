...
export default class Dom {
  ...
  /**
   * Correspondig DOM Document textarea element
   *
  ...
  /**
   * Allows read access to correspondig DOM Document textarea element
   *
  ...
  constructor(editor, document) {
    ...
    this.#textarea = this.document.documentElement;
  }
  ...
  insert(element) {
    ...
    if (
      ...
      this.editor.textarea.appendChild(element);
    } else {
      throw new Error(ErrorMessage.INVALID_ARGUMENT);
    }
  }
  ...
  contains(element) {
    ...
    if (this.editor.textarea.contains(element)) {
      return true;
    }

    const textarea = element.closest(this.editor.textarea.localName);

    return textarea && !textarea.parentElement;
  }
  ...
  wrap(element, name, opts = {}) {
    ...
    if (!(element instanceof HTMLElement)) {
      ...
      element.parentElement.localName !== name &&
      (sibling = this.closest(element, name))
    ) {
   ...
   */
  getSelectedAttributesByName(name, command) {
