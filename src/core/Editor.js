...
export default class Editor {
    ...
    constructor(orig, config = {}) {
        ...
        // Menubar
        this.#menubar = this.dom.createElement(TagName.MENUBAR, { attributes: { role: 'toolbar' } });
        ...
        this.focusbar.appendChild(this.dom.createElement(TagName.SPAN, { attributes: { class: 'trigger' } }));
        this.focusbar.appendChild(this.dom.createElement(TagName.DIV, { attributes: { class: 'trigger-content' } }));
            ...
            Object.entries(item.config).forEach(([key, val]) => {
                if(item.name === 'base') {
                    this.config[key] = config?.[key]
                        || this.constructor.defaultConfig?.[key]
                        || val;
                } else {
                    this.config.customPlugins ??= {};
                    this.config.customPlugins[item.name] ??= {};
                    this.config.customPlugins[item.name][key] = config.customPlugins[item.name]?.[key]
     ...
     */
    getHtml() {
        const textarea = this.dom.createElement(this.textarea.localName, { html: this.textarea.innerHTML });
