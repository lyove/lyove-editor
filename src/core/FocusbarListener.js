...
export default class FocusbarListener extends BarListener {
    ...
    constructor(editor) {
        ...
        const focusbarToolbox = Array.from(focusbar.children).find(e => e.className === "trigger-content");
    ...
    focusin(event) {
        if (event.target instanceof HTMLElement && event.target.hasAttribute('data-focusable')) {
            ...
            const focusbarToolbox = Array.from(this.editor.focusbar.children).find(e => e.className === "trigger-content");
    ...
    #show(focusElement) {
        ...
        Object.keys(focusElement.dataset).forEach(key => (focusbar.dataset[key] = focusElement.dataset[key]));
