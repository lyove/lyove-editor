...
export default class Command {
    ...
    insert(attributes = {}) {
        ...
        if (this.tag.group === TagGroup.FORMAT) {
            ...
            if (sameTagNameElement) {
                ...
                const newTagNameElement = this.editor.dom.createElement(this.tag.name, { attributes });
    ...
    openDropdown(command, triggerElement) {
        this.dropdown?.open((attributes) => this.insert(attributes), this.#selectedAttributes(command), triggerElement);
    }
        ...
        if (dropdowns.length > 0) {
            Array.from(dropdowns).forEach((item) => {
                ...
                if (thisParent) {
                    ...
                    const arrowElement = Array.from(targetButton.children).find((item) => item.className.indexOf("btn-arrow") > -1);
                    if (arrowElement) {
