...
export default class DropdownElement extends HTMLElement {
  ...
  constructor() {
    ...
    this.addEventListener(
      "keydown",
      (event) => event.key === Key.ESC && this.close()
    );
  ...
  show(triggerElement, thisEditor) {
    ...
    // Arrow style
    if (left <= 0) {
      const arrow = Array.from(this.children).find(
        (item) => item.role === "arrow"
      );
  ...
  clickedOutside(event) {
    ...
    if (dropdowns.length > 0) {
      ...
      const isTriggerBtn = (ele) => {
        if (ele) {
          if (
            ele.localName === "button" &&
            ele.dataset.command === thisCommand
          ) {
            matched = true;
            return true;
          } else {
      ...
      if (
        !(thisDropdown.contains(event.target) || isTriggerBtn(event.target))
      ) {
        this.close();
      }
    }
