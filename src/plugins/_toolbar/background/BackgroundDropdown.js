...
export default class BackgroundDropdown extends Dropdown {
    ...
    */
    _prepareContent() {
        ...
        const colorboard = this.editor.dom.createElement(TagName.DIV, { attributes: { class: "colorboard" } });

        color_groups.forEach((group) => {
            const groupTag = this.editor.dom.createElement(TagName.SPAN, { attributes: { class: "colorboard-group" } });
            group.forEach((item) => {
                const itemTag = this.editor.dom.createElement(TagName.SPAN, { attributes: { class: `colorboard-group-item${item.class ? ` ${item.class}` : ""}` } });
                const innerTag = this.editor.dom.createElement(TagName.SPAN, { attributes: { 'data-color': item.color } });
