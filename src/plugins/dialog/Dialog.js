...
export default class Dialog {
   ...
   * Form creator
   *
   * @type {DialogFormCreator|undefined}
   */
  #formCreator;
  ...
  open(save, attributes = {}) {
    this.url
      ? this.#openBrowser(save, attributes)
      : this.#openDialog(save, attributes);
  }
  ...
  #openDialog(save, attributes = {}) {
    ...
    this.#formCreator = new DialogFormCreator(
      this.editor.dom,
      this._("Save"),
      this._("Cancel"),
      close
    ...
    form.addEventListener("submit", (event) => {
      ...
      Array.from(form.elements).forEach(
        (item) => (data[item.name] = item.value)
      );
      save(data);
    });
    Object.entries(attributes).forEach(
      ([key, val]) => form.elements[key] && (form.elements[key].value = val)
    );
  ...
  cleanup() {
    Array.from(
      this.editor.element.getElementsByTagName(TagName.DIALOG)
    ).forEach((item) => {
