...
export default class TableListener extends Listener {
  ...
  keydown(event) {
    ...
    if (
      ...
      (base instanceof HTMLTableElement ||
        base instanceof HTMLTableSectionElement) &&
      ...
      const rowIndex =
        base instanceof HTMLTableElement ? row.rowIndex : row.sectionRowIndex;
      ...
      if (isNav) {
        if (event.key === Key.LEFT && !isFirst) {
          ...
          this.editor.dom.focusEnd(
            table.rows[table.rows.length - 1].cells[cellLength - 1]
          );
        } else if (event.key === Key.LEFT) {
          this.editor.dom.focusEnd(
            table.rows[row.rowIndex - 1].cells[cellLength - 1]
          );
        ...
        if (event.key === Key.LEFT && cellLength > 1 && isFirst) {
          Array.from(table.rows).forEach((item) =>
            item.appendChild(item.cells[cellIndex])
          );
        ...
        if (event.key === Key.LEFT) {
          Array.from(table.rows).forEach((item) =>
            this.#cell(item, item.cells[cellIndex])
          );
        } else if (event.key === Key.RIGHT) {
          Array.from(table.rows).forEach((item) =>
            this.#cell(item, item.cells[cellIndex + 1])
          );
        } else if (event.key === Key.UP) {
  ...
  #cell(element, ref = null) {
    const name =
      element.parentElement.localName === TagName.THEAD
        ? TagName.TH
        : TagName.TD;
   ...
   */
    ...
    const anc =
      sel.anchorNode instanceof HTMLElement ? sel.anchorNode : element;
    ...
    if (
      element.lastChild instanceof HTMLBRElement &&
    ...
    return (
      sel.anchorOffset === anc.textContent.length &&
      [element, last].includes(anc)
    );
  }
}
